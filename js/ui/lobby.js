...
export function renderLobby(state) {
  ...
  const players = state.players.map(player => `<div class="player-row">
    <div class="player-color elephant-icon" style="--player-color:${player.color}"></div>
  ...
  return `<div class="app page"><header class="topbar"><div class="brand"><div class="brand-mark">✦</div>Imaginarium Companion</div><button class="button button-ghost" data-action="leave-room">Выйти</button></header>
    ...
    <div class="lobby-layout"><section class="panel panel-padding"><strong>Игроки · ${state.players.length}/7</strong><div class="players-list">${players}</div></section>
    <aside class="lobby-side"><section class="panel panel-padding"><p class="lobby-note">Можно стартовать любым составом до 7 человек. Подключённые игроки должны быть в сети.</p></section>${isHost ? `<button class="button button-primary button-full" data-action="start-game" ${state.players.length < 1 || state.players.some(player => !player.connected) || state.busy ? "disabled" : ""}>Начать игру</button>` : `<div class="panel panel-padding muted">Ждём, когда хост начнёт игру.</div>`}</aside></div>
  </div>`;
}
