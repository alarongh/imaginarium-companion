...
export function renderResults(state) {
  ...
  return `<div class="app page"><header class="game-header"><div><span class="badge badge-accent">Итоги кона ${state.roundNumber}</span><h1 class="game-title">Результаты раскрыты</h1></div></header>${state.error ? `<div class="error-banner">${escapeHtml(state.error)}</div>` : ""}
    ...
    ${endVote(state)}<div class="results-actions">${isHost ? `<div class="panel panel-padding auto-next"><strong>Следующий кон запустится автоматически</strong><span class="muted">Можно начать сразу, если все уже посмотрели результаты.</span><button class="button button-primary button-full" data-action="next-round" ${state.endVote?.active || state.busy ? "disabled" : ""}>Начать сейчас</button></div>` : `<div class="panel panel-padding muted">Следующий кон запустится автоматически после короткой паузы.</div>`}</div></aside></div></div>`;
}
